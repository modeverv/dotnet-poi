...
- 24 unit tests for write/read round-trips (slide count, picture bytes, anchor, rotation, flip, deduplication, text boxes with formatting, slide size)
...
Scope note: Phase 3.3 covers the core XSLF write/read surface — slides, picture shapes with anchor/rotation/flip, picture data deduplication, and text boxes with rich text formatting. Animations, charts, and themes are later-phase work.
...
├── tests/
│   ├── DotnetPoi.Core.Tests/       # Core tests (226) — all formats
...
- write/read round-trip の 24 unit test（スライド数・画像バイト列・アンカー・回転・フリップ・重複排除・テキストボックス・スライドサイズ）
...
範囲の注意: Phase 3.3 が対象にしているのは XSLF の書き出し・読み込みコアサーフェス（スライド・アンカー付き画像・回転・フリップ・重複排除・テキストボックス）です。アニメーション・グラフ・テーマは後続フェーズの対象です。
...
- **公開可能な出力**: 生成 HTML は `docs/` に置く。Markdown 原稿、examples、生成スクリプト/設定は、静的サイトジェネレーターの都合がない限り `docs_src/` 外に置く。
...
├── tests/
│   ├── DotnetPoi.Core.Tests/       # Core テスト（226）— 全フォーマット
